...
namespace Stats
{
    ...
    {
        // QueryConnections can block (~30s); never hold mu on the UI thread.
        runOnNewThread([this] {
            // Call RPC without holding mu to avoid blocking
    ...
    void ConnectionLister::Loop(std::stop_token stopToken)
    {
        while (!stopToken.stop_requested())
        {
            // Sleep until the next poll, but wake immediately when the view
            // opens (SetInView) or we're shutting down (Stop). The stop check
            // shares waitMu_ with the wake path so the notification cannot be
            // lost between checking the token and entering the wait.
            if (!Throne::waitForStopOrTimeout(
                    waitCond_, waitMu_, stopToken,
                    inView_.load() ? kActivePollMs : kRelaxedPollMs)) return;
            if (suspend.load(std::memory_order_acquire) ||
                !Configs::dataManager->settingsRepo->enable_stats) continue;

            // Call RPC without holding mu to avoid blocking other threads
            libcore::QueryConnectionsResp resp = API::defaultClient->QueryConnections();
    ...
    void ConnectionLister::updateWithResponse(const libcore::QueryConnectionsResp& resp)
    {
        ...
        if (sort == Default)
        {
            ...
            runOnUiThread([window, toUpdate, toAdd] {
                if (!window) return;
                window->UpdateConnectionList(toUpdate, toAdd);
            });
        } else
        {
            if (sort == ByDownload)
            ...
            const QPointer<MainWindow> window(GetMainWindow());
            runOnUiThread([window, sorted] {
                if (!window) return;
                window->UpdateConnectionListWithRecreate(sorted);
            });
        }
    }
    ...
    ConnectionSort ConnectionLister::getSort() const
    {
        QMutexLocker lock(&mu);
        return sort;
    }
