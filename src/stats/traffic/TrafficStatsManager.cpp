...
namespace Stats {
    ...
    void TrafficStatsManager::Init() {
        ...
        worker.Start([this](std::stop_token stopToken) {
            runLoop(stopToken);
        });
    }

    void TrafficStatsManager::Stop() {
        worker.Stop([this] {
            QMutexLocker lock(&waitMu);
            waitCondition.wakeAll();
        });
        started.store(false, std::memory_order_release);
    }

    void TrafficStatsManager::runLoop(std::stop_token stopToken) {
        ...
        while (!stopToken.stop_requested()) {
            if (!Throne::waitForStopOrTimeout(waitCondition, waitMu, stopToken, kRollupIntervalMs)) break;
            runRollupOnce();
        }
    }
