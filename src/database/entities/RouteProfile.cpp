...
namespace Configs {
    ...
    QJsonArray RouteProfile::get_route_rules(bool forView, std::map<int, QString> outboundMap) {
        ...
        for (const auto &item: Rules) {
            ...
            // Only inject when the local .srs exists; otherwise core start would reference
            // a missing rule-set tag. Download happens off the start path.
            if (!added_adblock && Configs::dataManager->settingsRepo->adblock_enable
                && adblockRulesetAvailable() && rule_json["action"] == "route") {
                res += createAdblockRule();
                added_adblock = true;
            }
            res += rule_json;
        }
        if (!added_adblock && Configs::dataManager->settingsRepo->adblock_enable && adblockRulesetAvailable())
            res += createAdblockRule();
