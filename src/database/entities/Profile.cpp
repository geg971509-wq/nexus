...
namespace Configs
{
    Profile::Profile(const std::shared_ptr<Configs::outbound> &outbound, const QString &type_)
    {
    ...
    QString Profile::DisplayTestResult() const {
        ...
        QString result;
        // Prefer resolved egress country; if missing (URL latency-only tests),
        // infer a 2-letter code from the profile/outbound name ("TW - ...").
        QString country = test_country;
        if (country.isEmpty() && outbound) country = InferCountryCode(outbound->name);
        if (country.isEmpty()) country = InferCountryCode(name);
        if (!country.isEmpty()) {
            const QString flag = CountryCodeToFlag(country);
            if (!flag.isEmpty()) result += UNICODE_LRO + flag + " ";
        }
        if (latency < 0) {
            result = "Unavailable";
            return result;
        ...
        // Egress IP lives in Address column only; keep Test Result as flag + ms (+ speed).
        return result;
    }
    ...
    QString Profile::DisplayTraffic() const {
        const qint64 up = traffic_uplink.load(std::memory_order_relaxed);
        const qint64 down = traffic_downlink.load(std::memory_order_relaxed);
        if (down + up == 0) return "";
        return UNICODE_LRO + QString("%1↑ %2↓").arg(ReadableSize(up), ReadableSize(down));
    }

    void Profile::ResetTraffic() {
        traffic_downlink.store(0, std::memory_order_relaxed);
        traffic_uplink.store(0, std::memory_order_relaxed);
    }
