                                  ...
                                  [&](int a, int b) {
                                      ...
                                      if (sortAction.method == GroupSortMethod::ByType) {
                                          ...
                                          if (test_sort_by == testBy::latency) {
                                              int latA = get_latency_for_sort(profA);
                                              int latB = get_latency_for_sort(profB);
                                              if (latA == latB) return false;
                                              return sortAction.descending ? latA > latB : latA < latB;
                                          }
                                          if (test_sort_by == testBy::dlSpeed) {
                                              double speedA = bitrateToBps(profA->dl_speed);
                                              double speedB = bitrateToBps(profB->dl_speed);
                                              if (speedA == speedB) return false;
                                              return sortAction.descending ? speedA > speedB : speedA < speedB;
                                          }
                                          if (test_sort_by == testBy::ulSpeed) {
                                              double speedA = bitrateToBps(profA->ul_speed);
                                              double speedB = bitrateToBps(profB->ul_speed);
                                              if (speedA == speedB) return false;
                                              return sortAction.descending ? speedA > speedB : speedA < speedB;
                                          }
                                          if (test_sort_by == testBy::ipOut) {
                                              if (profA->ip_out == profB->ip_out) return false;
                                          ...
                                          if (traffic_sort_by == trafficBy::total) {
                                              const auto totalA = profA->traffic_downlink.load(std::memory_order_relaxed)
                                                                  + profA->traffic_uplink.load(std::memory_order_relaxed);
                                              const auto totalB = profB->traffic_downlink.load(std::memory_order_relaxed)
                                                                  + profB->traffic_uplink.load(std::memory_order_relaxed);
                                              if (totalA == totalB) return false;
                                              return sortAction.descending ? totalA > totalB  : totalA < totalB;
                                          }
                                          if (traffic_sort_by == trafficBy::dl) {
                                              const auto a = profA->traffic_downlink.load(std::memory_order_relaxed);
                                              const auto b = profB->traffic_downlink.load(std::memory_order_relaxed);
                                              if (a == b) return false;
                                              return sortAction.descending ? a > b : a < b;
                                          }
                                          if (traffic_sort_by == trafficBy::ul) {
                                              const auto a = profA->traffic_uplink.load(std::memory_order_relaxed);
                                              const auto b = profB->traffic_uplink.load(std::memory_order_relaxed);
                                              if (a == b) return false;
                                              return sortAction.descending ? a > b : a < b;
                                          }
                                      }
                                      if (ms_a == ms_b) return false;
