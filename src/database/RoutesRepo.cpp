...
namespace Configs {
    ...
    void RoutesRepo::DeleteRouteProfile(int id) {
        ...
        try {
            ...
            qWarning() << "RoutesRepo::DeleteRouteProfile failed for id" << id << ":" << e.what();
        }
    }

    void RoutesRepo::UpdateRouteProfiles(const QList<std::shared_ptr<RouteProfile>>& routeProfiles) {
        // Drain query first, release DB lock before acquiring mutex
        ...
        for (const auto& routeProfile : routeProfiles) {
            ...
            try {
                ...
                qWarning() << "RoutesRepo::UpdateRouteProfiles failed for route" << routeProfile->id << ":" << e.what();
            }
        }
        ...
        if (!toDelete.empty()) {
            try {
                ...
                qWarning() << "RoutesRepo::UpdateRouteProfiles delete failed:" << e.what();
            }
        }
    }
    ...
    QList<std::shared_ptr<RouteProfile>> RoutesRepo::GetAllRouteProfiles() const {
        ...
        // Drain query first, build profile objects before acquiring mutex
        std::vector<std::shared_ptr<RouteProfile>> rows;
        {
            ...
            while (profileQuery.tryStep()) {
                rows.push_back(routeProfileFromProfileRow(*profileQuery));
            }
        }
