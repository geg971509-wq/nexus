...
namespace Configs {
    void initDB(const std::string& dbPath) {
        ...
        if (dataManager->groupsRepo->GetAllGroupIds().empty()) {
            ...
            if (!dataManager->groupsRepo->AddGroup(defaultGroup)) {
                qWarning() << "Failed to create the default group; the database may be unwritable";
            }
        }
    ...
    QString FindCoreRealPath() {
        return ResolveCorePath(QApplication::applicationDirPath());
    }
