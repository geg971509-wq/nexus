...
namespace Configs {
    ...
    void CalculatePrerequisities(std::shared_ptr<BuildSingBoxConfigContext> &ctx) {
        const auto *s = Configs::dataManager->settingsRepo.get();
        ctx->tunEnabled = s->spmode_vpn;
        ctx->os = getOS();
        if (ctx->os == Linux)
        {
        ...
        // Get route chain
        auto routeChain = Configs::dataManager->routesRepo->GetRouteProfile(s->current_route_id);
        ...
        // WARP is a system underlay (standalone warp-client), not an outbound hop.
        // Legacy route profiles that still reference warp-bypass resolve to proxy.
        preReqs->routingDeps->outboundMap[warpBypassID] = "proxy";
        int suffix = 0;
        ...
        if (auto group = Configs::dataManager->groupsRepo->GetGroup(ctx->ent->gid); group != nullptr)
        {
            ...
            for (const auto id : groupEnts) {
                if (auto ent = Configs::dataManager->profilesRepo->GetProfile(id);
                    ent != nullptr && usesXrayCore(ent)) {
                    ctx->proxyUsesXray = true;
                    break;
                }
            }
        ...
        // Hijack
        if (s->enable_dns_server) {
            for (const auto& rule : s->dns_server_rules) {
    ...
    void buildNTPSection(std::shared_ptr<BuildSingBoxConfigContext> &ctx) {
        ...
        if (s->enable_ntp) {
            QJsonObject ntpObj;
            ntpObj["enabled"] = true;
            ntpObj["server"] = s->ntp_server_address;
            ntpObj["server_port"] = s->ntp_server_port;
            ntpObj["interval"] = s->ntp_interval;
            const QString ntpDetour =
                (s->ntp_outbound == "proxy" && !ctx->forTest)
                    ? "proxy"
                    : "direct";
    ...
    QJsonObject buildDnsObj(QString address, std::shared_ptr<BuildSingBoxConfigContext> &ctx) {
        ...
        if (addr.contains(":")) {
            // Only host:port (a single colon) carries a port suffix; a bare IPv6
            // address (e.g. tls://2001:4860:4860::8888) has multiple colons and
            // must be kept whole.
            if (addr.count(':') == 1) {
                const auto spl = addr.split(":");
                bool ok = false;
                const int p = spl[1].toInt(&ok);
                if (ok) {
                    addr = spl[0];
                    port = p;
                }
    ...
    void buildDNSSection(std::shared_ptr<BuildSingBoxConfigContext> &ctx, bool useDnsObj) {
        auto *s = Configs::dataManager->settingsRepo.get();
        // macOS Tun cannot use type=local; empty underlying DNS used to hard-fail.
        // Auto-fill a public resolver so Tun starts; user can still override in Routing.
        if (getOS() == Darwin && s->core_box_underlying_dns.isEmpty() && s->spmode_vpn)
        {
            s->core_box_underlying_dns = QStringLiteral("1.1.1.1");
        }

        if (s->use_dns_object && useDnsObj) {
            ctx->buildConfigResult->coreConfig["dns"] = QString2QJsonObject(s->dns_object);
            return;
        }
        ...
        // remote
        if (!ctx->forTest) {
            auto remoteDnsObj = buildDnsObj(s->remote_dns, ctx);
            // dns-remote always detours via proxy. Plain UDP/QUIC DNS dies on
            // HTTP outbounds (and Xray UDP is flaky); DoH rides TCP/TLS instead.
            const auto remoteDnsType = remoteDnsObj.value("type").toString();
            if (remoteDnsType == "udp" || remoteDnsType == "quic") {
        ...
        // direct
        auto directDnsObj = buildDnsObj(s->direct_dns, ctx);
        ...
        if (!ctx->forTest && ctx->proxyUsesXray) {
            rules += QJsonObject{
                    ...
                    {"action", "route"},
                    {"strategy", s->direct_dns_strategy},
                    {"server", "dns-direct"},
                };
        }
        ...
        // HijackRules
        if (s->enable_dns_server && !ctx->forTest && !hijackDeps->noConditions())
        {
            ...
            auto addHijackRules = [&](const QJsonObject &conditions) {
                ...
                v4["rcode"] = "NOERROR";
                v4["answer"] = QString("* IN A %1").arg(s->dns_v4_resp);
                rules += v4;

                if (s->dns_v6_resp.isEmpty()) return;
                auto v6 = conditions;
                v6["query_type"] = "AAAA";
                v6["action"] = "predefined";
                v6["rcode"] = "NOERROR";
                v6["answer"] = QString("* IN AAAA %1").arg(s->dns_v6_resp);
                rules += v6;
            };
        ...
        const bool useDirectFinalDNS = s->dns_final_out == "direct";
        ...
        if (dnsDeps->needProxyDnsRules && useDirectFinalDNS) {
            ...
            if (!dnsDeps->proxyRuleSets.isEmpty()) {
                rules += QJsonObject{
                        ...
                        {"action", "route"},
                        {"strategy", s->remote_dns_strategy},
                        {"server", "dns-remote"},
                    };
            }
        ...
        // final rule: proxy
        auto finalStrategy = useDirectFinalDNS ? s->direct_dns_strategy : s->remote_dns_strategy;
        ...
        // Local
        auto dnsLocalAddress = s->core_box_underlying_dns.isEmpty() ? "local" : s->core_box_underlying_dns;
        ...
        auto dnsObj = QJsonObject{
            {"servers", servers},
            {"rules", rules},
            {"cache_capacity", s->dns_cache_capacity},
        };
        if (s->dns_disable_cache) dnsObj["disable_cache"] = true;
        if (s->dns_disable_expire) dnsObj["disable_expire"] = true;
        if (s->dns_reverse_mapping) dnsObj["reverse_mapping"] = true;
    ...
    void buildInboundSection(std::shared_ptr<BuildSingBoxConfigContext> &ctx) {
        ...
        if (!s->disable_mixed_inbound) {
            inboundObj["tag"] = "mixed-in";
            inboundObj["type"] = "mixed";
            inboundObj["listen"] = s->inbound_address;
            inboundObj["listen_port"] = s->inbound_socks_port;
            if (s->inbound_auth) {
                inboundObj["users"] = QJsonArray{
                    QJsonObject{
                                            {"username", s->inbound_user},
                                            {"password", s->inbound_pass}
                    }
                };
            }
            inbounds += inboundObj;
        ...
        if (s->spmode_vpn) {
            ...
            inboundObj["mtu"] = s->vpn_mtu;
            inboundObj["stack"] = s->vpn_implementation;
            inboundObj["strict_route"] = s->vpn_strict_route;
            if (ctx->os == Linux && s->vpn_auto_redirect) inboundObj["auto_redirect"] = true;
            const auto tunIPv4CIDR = s->vpn_tun_ipv4_cidr;
            const auto tunIPv6CIDR = s->vpn_tun_ipv6_cidr;
            ctx->buildConfigResult->tunIPv4CIDR = tunIPv4CIDR;
            auto tunAddress = QJsonArray{tunIPv4CIDR};
            if (s->vpn_ipv6) tunAddress += tunIPv6CIDR;
            inboundObj["address"] = tunAddress;

            QJsonArray routeExcludeAddrs;
            if (s->private_range_bypass) {
                // Do NOT set route_address to the Tun CIDR: sing-tun treats it as
                // an allowlist and replaces full auto_route with only that prefix
                // (public traffic then leaks via en0). Carve Tun out of private
                // excludes so system DNS (Tun+1) stays on the interface while LAN
                // still bypasses.
                routeExcludeAddrs = TunPrivateBypass::privateRangeBypassExcludingTun(tunIPv4CIDR);
                if (s->vpn_ipv6) {
                    for (const auto& item : TunPrivateBypass::privateRangeBypassExcludingTunV6(tunIPv6CIDR))
                        routeExcludeAddrs << item;
                }
            }
            QJsonArray routeExcludeSets;
            if (s->enable_tun_routing)
            {
        ...
        inbounds.prepend(QJsonObject{
            ...
            {"type", "direct"},
            {"listen", "127.0.0.1"},
            {"listen_port", s->core_dns_in_port}
        });

        // Hijack
        if (s->enable_redirect) {
            inbounds.prepend(QJsonObject{
                {"tag", "hijack"},
                {"type", "direct"},
                {"listen", s->redirect_listen_address},
                {"listen_port", s->redirect_listen_port},
            });
        }
        if (s->enable_dns_server) {
            inbounds.prepend(QJsonObject{
                {"tag", "hijack-dns"},
                {"type", "direct"},
                {"listen", s->dns_server_listen_lan ? "0.0.0.0" : "127.1.1.1"},
                {"listen_port", s->dns_server_listen_port},
            });
        }

        // custom
        QJSONARRAY_ADD(inbounds, QString2QJsonObject(s->custom_inbound)["inbounds"].toArray())
    ...
    void buildOutboundChain(std::shared_ptr<BuildSingBoxConfigContext> &ctx, const QList<int>& entIDs, const QString& prefix, bool includeProxy, bool link, int singToXrayPort = -1, int xrayToSingPort = -1, int startSuffix = 0)
    {
        ...
        if (!initialSingEnts.isEmpty()) {
            buildSingboxChain(ctx, initialSingEnts, prefix, includeProxy, link, startSuffix, false);
        }
        if (!xrayEnts.isEmpty()) {
    ...
    void buildOutboundsSection(std::shared_ptr<BuildSingBoxConfigContext> &ctx) {
        ...
        {
            buildAutoSelectorGroup(ctx, group);
            if (!ctx->error.isEmpty()) return;
        }
        else
        {
    ...
    void buildRouteSection(std::shared_ptr<BuildSingBoxConfigContext> &ctx) {
        ...
        if (!routeChain->isRaw) {
            ...
            if (!s->resolve_domain_strategy.isEmpty()) {
                injected.resolve = QJsonObject{
                    {"inbound", QJsonArray{"mixed-in", "tun-in"}},
                    {"action", "resolve"},
                    {"strategy", s->resolve_domain_strategy},
                };
            }
            ...
            if (s->enable_redirect && !ctx->forTest) {
        ...
        // rulesets
        // Prefer proxy detour for remote rule-set fetches: core start often races
        // direct dial (EBADF / blocked GitHub) before the tunnel path is usable.
        const bool useProxyDetour = !ctx->forTest;
        auto ruleSetArray = QJsonArray();
        auto appendRemoteRuleSet = [&](const QString &tag, const QString &url) {
            QJsonObject obj{
                {"type", "remote"},
                {"tag", tag},
                {"format", "binary"},
            ...
            if (useProxyDetour) obj.insert("download_detour", "proxy");
            ruleSetArray += obj;
        };
        for (const auto &item: routeDeps->neededRuleSets) {
            if(auto url = QUrl(item); url.isValid() && url.fileName().contains(".srs")) {
                appendRemoteRuleSet(get_rule_set_name(item), item);
            }
            else
                if (auto url = ruleSetUrl(item.toStdString()); !url.empty()) {
                    appendRemoteRuleSet(item, get_jsdelivr_link(QString::fromUtf8(url.data(), url.size())));
                }
        }

        // AdBlock: local only at start. Remote fetch through proxy during initialize
        // races an unready outbound (EBADF). Missing file => skip; UI downloads later.
        if (s->adblock_enable && adblockRulesetAvailable()) {
            ruleSetArray += QJsonObject{
                {"type", "local"},
                {"tag", QStringLiteral("throne-adblocksingbox")},
                {"format", "binary"},
        ...
        if (routeChain->isRaw) {
            ...
            // System WARP is underlay; legacy warp-bypass routes map to proxy.
            route["final"] = "proxy";
        } else {
            route["final"] = outboundIDToString(defOut);
        }
        if (s->enable_stats && !route.contains("find_process"))  route["find_process"] = true;
        if (!route.contains("default_domain_resolver"))
            route["default_domain_resolver"] = QJsonObject{
                                    {"server", "dns-direct"},
                                    {"strategy", s->default_domain_strategy}};
        if (s->spmode_vpn && !route.contains("auto_detect_interface")) route["auto_detect_interface"] = true;
        // Pin proxy egress to the WARP underlay so the stack is genuinely two-layer.
        // auto_detect_interface alone lands on the physical interface: sing-box's
        // Darwin monitor only accepts a 0.0.0.0/0 route carrying RTF_GATEWAY, WARP
        // never installs one (it splits into /1s rather than seizing the default),
        // so egress would be pinned to en0 with IP_BOUND_IF and skip WARP entirely.
        // default_interface wins over auto_detect_interface in the dialer, and
        // auto_detect_interface stays as the fallback for when WARP is down.
        // Caller supplies the device name from the 2s poll cache — never probe here
        // (UI-thread fork freezes the event loop the admin password sheet needs).
        if (ctx->tunEnabled && !ctx->warpUnderlayInterface.isEmpty()
            && !route.contains("default_interface"))
            route["default_interface"] = ctx->warpUnderlayInterface;
    ...
    std::shared_ptr<BuildConfigResult> BuildSingBoxConfig(const std::shared_ptr<Profile>& ent, bool forExport,
        ...
        ctx->warpUnderlayInterface = warpUnderlayInterface;
