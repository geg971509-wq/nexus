...
namespace Configs {
    bool TcpBrutal::ParseFromLink(const QString& link)
    {
        ...
        if (query.hasQueryItem("brutal_up_mbps")) up_mbps = parseIntOr(query.queryItemValue("brutal_up_mbps"), up_mbps);
        if (query.hasQueryItem("brutal_down_mbps")) down_mbps = parseIntOr(query.queryItemValue("brutal_down_mbps"), down_mbps);
        return true;
    }
    ...
    QString TcpBrutal::ExportToLink() const
    {
        QUrlQuery query;
        if (!enabled) return "";
    ...
    QJsonObject TcpBrutal::ExportToJson() const
    {
        QJsonObject object;
    ...
    BuildResult TcpBrutal::Build() const
    {
        return {ExportToJson(), ""};
    }
    ...
    bool Multiplex::ParseFromLink(const QString& link)
    {
        ...
        if (query.hasQueryItem("mux_max_connections")) max_connections = parseIntOr(query.queryItemValue("mux_max_connections"), max_connections);
        if (query.hasQueryItem("mux_min_streams")) min_streams = parseIntOr(query.queryItemValue("mux_min_streams"), min_streams);
        if (query.hasQueryItem("mux_max_streams")) max_streams = parseIntOr(query.queryItemValue("mux_max_streams"), max_streams);
    ...
    QString Multiplex::ExportToLink() const
    {
        QUrlQuery query;
    ...
    QJsonObject Multiplex::ExportToJson() const
    {
        QJsonObject object;
