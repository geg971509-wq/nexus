...
namespace Configs {
    bool xrayMultiplex::ParseFromLink(const QString &link) {
        ...
        if (query.hasQueryItem("mux_concurrency")) concurrency = parseIntOr(query.queryItemValue("mux_concurrency"), concurrency);
        if (query.hasQueryItem("mux_xudp_concurrency")) xudpConcurrency = parseIntOr(query.queryItemValue("mux_xudp_concurrency"), xudpConcurrency);
        return true;
    }
    ...
    QString xrayMultiplex::ExportToLink() const {
        QUrlQuery query;
    ...
    QJsonObject xrayMultiplex::ExportToJson() const {
        QJsonObject object;
