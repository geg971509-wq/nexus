...
namespace Configs {
    ...
    SecurityInfo outbound::SecurityFromTLS(const QString& transport) const
    {
        SecurityInfo info;
    ...
    SecurityInfo outbound::GetSecurity() const
    {
        if (IsXray()) {
    ...
    QString outbound::DisplaySecurity() const
    {
    ...
    bool outbound::ParseFromLink(const QString& link)
    {
        ...
        if (!url.isValid())
        {
            ...
            server_port = url.port(0); // Use 0 as default instead of -1
        }
    ...
    bool outbound::ParseFromJson(const QJsonObject& object)
    {
        ...
        if (object.contains("server_port")) {
            server_port = object["server_port"].toInt();
            // Reject out-of-range ports from untrusted JSON explicitly instead
            // of letting ExportToJson/Build silently drop the field downstream.
            if (server_port < 1 || server_port > 65535) {
                qWarning() << "Invalid server_port in outbound JSON, dropping:" << object["server_port"];
                server_port = 0;
            }
        }
    ...
    QString outbound::ExportToLink() const
    {
        QUrlQuery query;
        mergeUrlQuery(query, dialFields->ExportToLink());
        return query.toString();
    }
    QJsonObject outbound::ExportToJson() const
    {
        QJsonObject object;
    ...
    QJsonObject outbound::ExportIdentity() const
    {
    ...
    BuildResult outbound::Build() const
    {
        QJsonObject object;
        if (!server.isEmpty()) object["server"] = server;
        if (server_port > 0) object["server_port"] = server_port;
