...
namespace Configs {
    ...
    QString shadowsocks::ExportToLink() const
    {
        QUrl url;
        QUrlQuery query;
        url.setScheme("ss");
    ...
    QJsonObject shadowsocks::ExportToJson() const
    {
        QJsonObject object;
    ...
    BuildResult shadowsocks::Build() const
    {
        // Historically this split plugin into plugin/plugin_opts as an in-place
        // side effect; Build() is const now, so the split uses local copies.
        QString effPlugin = plugin;
        QString effPluginOpts = plugin_opts;
        if (effPlugin.contains(";")) {
            effPluginOpts = SubStrAfter(effPlugin, ";");
            effPlugin = SubStrBefore(effPlugin, ";");
        }
        QJsonObject object;
        object["type"] = "shadowsocks";
    ...
    QString shadowsocks::DisplayType() const
    {
        return "Shadowsocks";
    }

    SecurityInfo shadowsocks::GetSecurity() const
    {
        SecurityInfo info;
