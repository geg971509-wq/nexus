...
namespace Configs {
    bool vmess::ParseFromLink(const QString& link)
    {
        ...
        if (!linkN.isEmpty()) {
            ...
            if (!objN.isEmpty()) {
                ...
                bool ok = false;
                server_port = objN["port"].toVariant().toInt(&ok);
                // A missing/out-of-range port makes the node unusable; reject it.
                if (!ok || server_port < 1 || server_port > 65535) return false;
                name = objN["ps"].toString();
                alter_id = objN["aid"].toVariant().toInt(&ok);
                if (!ok) alter_id = 0;
        ...
        if (query.hasQueryItem("alterId")) alter_id = parseIntOr(query.queryItemValue("alterId"), alter_id);
    ...
    QString vmess::ExportToLink() const
    {
        QUrl url;
        QUrlQuery query;
        url.setScheme("vmess");
    ...
    QJsonObject vmess::ExportToJson() const
    {
        QJsonObject object;
        object["type"] = "vmess";
    ...
    QString vmess::DisplayType() const
    {
        return "VMess";
    }

    SecurityInfo vmess::GetSecurity() const
    {
