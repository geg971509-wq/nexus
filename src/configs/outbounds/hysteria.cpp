...
namespace Configs {
    QStringList portsToPorts(const QStringList &ports) {
        ...
        for (const QString &v : ports) {
            ...
            if (range.size() == 2) {
                bool ok1, ok2;
                int port1 = range[0].toInt(&ok1);
                int port2 = range[1].toInt(&ok2);
                if (ok1 && ok2 && IsValidPort(port1) && IsValidPort(port2)) {
    ...
    bool hysteria::ParseFromLink(const QString& link)
    {
        ...
        if (url.scheme() == "hysteria") {
            ...
            if (query.hasQueryItem("recv_window_conn")) recv_window_conn = parseIntOr(query.queryItemValue("recv_window_conn"), recv_window_conn);
            if (query.hasQueryItem("recv_window")) recv_window = parseIntOr(query.queryItemValue("recv_window"), recv_window);
        ...
        if (query.hasQueryItem("upmbps")) up_mbps = parseIntOr(query.queryItemValue("upmbps"), up_mbps);
        if (query.hasQueryItem("downmbps")) down_mbps = parseIntOr(query.queryItemValue("downmbps"), down_mbps);
        ...
        if (server_port == 0 && server_ports.isEmpty()) server_port = DEFAULT_TLS_PORT;
    ...
    QString hysteria::ExportToLink() const
    {
        QUrl url;
        QUrlQuery query;
    ...
    QJsonObject hysteria::ExportToJson() const
    {
        QJsonObject object;
    ...
    QJsonObject hysteria::ExportIdentity() const
    {
    ...
    BuildResult hysteria::Build() const
    {
        QJsonObject object;
        object["type"] = protocol_version == "1" ? "hysteria" : "hysteria2";
    ...
    QString hysteria::DisplayType() const
    {
        return "Hysteria";
    }

    SecurityInfo hysteria::GetSecurity() const
    {
