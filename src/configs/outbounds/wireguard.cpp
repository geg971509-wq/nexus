...
namespace {
    // Amnezia field tables shared by the ini-file and wg:// URL parsers in
    // wireguard::ParseFromLink. Keys are lowercase; the ini parser matches
    // case-insensitively ("Jc" vs "jc"), the URL query keys are lowercase.
    struct AmneziaIntField {
        const char* key;
        int Configs::wireguard::* member;
    };
    constexpr AmneziaIntField amneziaIntFields[] = {
        {"jc", &Configs::wireguard::jc},
        {"jmin", &Configs::wireguard::jmin},
        {"jmax", &Configs::wireguard::jmax},
        {"s1", &Configs::wireguard::s1},
        {"s2", &Configs::wireguard::s2},
        {"s3", &Configs::wireguard::s3},
        {"s4", &Configs::wireguard::s4},
    };
    struct AmneziaStrField {
        const char* key;
        QString Configs::wireguard::* member;
    };
    constexpr AmneziaStrField amneziaStrFields[] = {
        {"h1", &Configs::wireguard::h1},
        {"h2", &Configs::wireguard::h2},
        {"h3", &Configs::wireguard::h3},
        {"h4", &Configs::wireguard::h4},
        {"i1", &Configs::wireguard::i1},
        {"i2", &Configs::wireguard::i2},
        {"i3", &Configs::wireguard::i3},
        {"i4", &Configs::wireguard::i4},
        {"i5", &Configs::wireguard::i5},
        // AmneziaWG 3.0
        {"header_protection_key", &Configs::wireguard::header_protection_key},
        {"content_padding_addition", &Configs::wireguard::content_padding_addition},
        {"rekey_after_time", &Configs::wireguard::rekey_after_time},
        {"rekey_timeout", &Configs::wireguard::rekey_timeout},
        {"reject_after_time", &Configs::wireguard::reject_after_time},
        {"keepalive_timeout", &Configs::wireguard::keepalive_timeout},
        {"max_handshake_attempts", &Configs::wireguard::max_handshake_attempts},
    };

    // Matches an ini key against a table key, ignoring case and underscores, so
    // the ini spelling ("HeaderProtectionKey") hits the same entry as the URL
    // query spelling ("header_protection_key").
    bool iniKeyMatches(const QString& iniKey, const char* tableKey)
    {
        return QString(iniKey).remove('_').compare(
                   QString::fromLatin1(tableKey).remove('_'), Qt::CaseInsensitive) == 0;
    }
}
...
namespace Configs {
    ...
    QString Peer::ExportToLink() const
    {
        QUrlQuery query;
    ...
    QJsonObject Peer::ExportToJson() const
    {
        QJsonObject object;
    ...
    bool wireguard::ParseFromLink(const QString& link)
    {
        ...
        if (link.contains("[Interface]") && link.contains("[Peer]")) {
            ...
            for (const auto& line : lines) {
                ...
                if (key == "MTU") mtu = parseIntOr(value, mtu);
                ...
                if (key == "Endpoint") {
                    // Parse endpoint as URL to handle IPv6 correctly
                    QUrl endpointUrl("udp://" + value);
                    peer->address = endpointUrl.host();
                    peer->port = endpointUrl.port(51820); // Default WireGuard port
                    server = peer->address;
                    server_port = peer->port;
                }
                for (const auto& field : amneziaIntFields) {
                    if (iniKeyMatches(key, field.key)) {
                        if (const auto v = parseIntOpt(value)) {
                            this->*(field.member) = *v;
                            enable_amnezia = true;
                        }
                    }
                }
                for (const auto& field : amneziaStrFields) {
                    if (iniKeyMatches(key, field.key)) {
        ...
        if (!outbound::ParseFromLink(link)) return false;
        ...
        if (query.hasQueryItem("mtu")) mtu = parseIntOr(query.queryItemValue("mtu"), mtu);
        if (query.hasQueryItem("use_system_interface")) system = query.queryItemValue("use_system_interface") == "true";
        if (query.hasQueryItem("workers")) worker_count = parseIntOr(query.queryItemValue("workers"), worker_count);
        ...
        for (const auto& field : amneziaIntFields) {
            if (query.hasQueryItem(field.key)) {
                if (const auto v = parseIntOpt(query.queryItemValue(field.key))) {
                    this->*(field.member) = *v;
                    enable_amnezia = true;
                }
            }
        ...
        for (const auto& field : amneziaStrFields) {
            if (query.hasQueryItem(field.key)) {
                this->*(field.member) = query.queryItemValue(field.key);
                enable_amnezia = true;
            }
        }
        FixAddress();
    ...
    bool wireguard::ParseFromJson(const QJsonObject& object)
    {
        ...
        if (object.value("warp_account").isObject()) {
            warp_account.fromJson(object.value("warp_account").toObject());
        }
        FixAddress();
        return true;
    }

    QString wireguard::ExportToLink() const
    {
        QUrl url;
        QUrlQuery query;
        url.setScheme("wg");
    ...
    QJsonObject wireguard::ExportToJson() const
    {
        QJsonObject object;
    ...
    QJsonObject wireguard::ExportToStorageJson() const
    {
        auto object = ExportToJson();
        if (auto account = warp_account.toJson(); !account.isEmpty()) {
            object["warp_account"] = account;
        }
        return object;
    }

    BuildResult wireguard::Build() const
    {
        QJsonObject object;
        object["type"] = "wireguard";
        if (!name.isEmpty()) object["tag"] = name;
    ...
    QString wireguard::GetAddress() const {
        return peer->address;
    }
    ...
    QString wireguard::GetPort() const {
        return QString::number(peer->port);
    }

    QString wireguard::DisplayAddress() const
    {
        return ::DisplayAddress(peer->address, peer->port);
    }

    QString wireguard::DisplayType() const
    {
        return warp_account.hasCredentials() ? "WARP (WireGuard)" : "WireGuard";
    }

    SecurityInfo wireguard::GetSecurity() const
    {
        return {QObject::tr("Encrypted"), enable_amnezia ? "AmneziaWG" : QString(), SecurityLevel::Secure};
    }

    bool wireguard::IsEndpoint() const
    {
        return true;
    }

    QJsonObject wireguard::AmneziaToJson() const
    {
        QJsonObject object;
