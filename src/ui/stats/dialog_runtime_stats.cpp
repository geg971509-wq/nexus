...
void DialogRuntimeStats::refreshLive() {
    ...
    if (Stats::trafficLooper) {
        const auto s = Stats::trafficLooper->GetRateSnapshot();
        ui->vSpeedProxy->setText(QStringLiteral("↓ %1   ↑ %2").arg(rate(s.proxy_downlink), rate(s.proxy_uplink)));
        ui->vSpeedDirect->setText(QStringLiteral("↓ %1   ↑ %2").arg(rate(s.direct_downlink), rate(s.direct_uplink)));
    } else {
