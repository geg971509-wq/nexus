...
QVariant ProfilesTableModel::data(const QModelIndex &index, int role) const {
    ...
    if (role == Qt::DisplayRole) {
        ...
        case ColAddress: {
            // Display-only: show egress IP:port when resolved; dial host stays in config.
            if (!profile->ip_out.isEmpty() && profile->outbound) {
                return DisplayAddress(profile->ip_out, profile->outbound->GetPort().toInt());
            }
...
void ProfilesTableModel::retranslateHeaders() {
...
void ProfilesTableModel::emplaceProfiles(int row1, int row2) {
    const int count = m_profileIds.size();
    // A drop above the first row arrives as row2 == -1, so it lands at index 0.
    if (row1 < 0 || row1 >= count || row2 < -1 || row2 >= count) return;
    // Moving a row onto itself (or just above itself) leaves the order unchanged,
    // and beginMoveRows rejects a destination inside the moved range.
    if (row2 == row1 || row2 == row1 - 1) return;

    beginMoveRows(QModelIndex(), row1, row1, QModelIndex(), row2 + 1);
    ...
    const int to = std::min(std::max(row1, row2), count - 1);
