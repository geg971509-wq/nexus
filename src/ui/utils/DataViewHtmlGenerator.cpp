...
QString DataViewHtmlGenerator::downloadSectionHtml() {
    ...
    return QString("<p style='text-align:center;margin:0;'>%1</p>")
...
QString DataViewHtmlGenerator::speedtestSectionHtml() {
    if (speedtest_.kind == SpeedtestPanelState::Kind::Speed) {
        auto firstLine = QCoreApplication::translate(
            "DataViewHtmlGenerator",
            "Running Speedtest: %1").arg(speedtest_.profileName);
        ...
        const auto serverLine = QCoreApplication::translate(
            "DataViewHtmlGenerator",
            "Server: %1%2, %3")
            .arg(speedtest_.serverCountryFlag, speedtest_.serverCountry, speedtest_.serverName);
        return QString(
           ...
           "</div>"
           "<p style='text-align:center;margin:0;'>%4</p>")
            .arg(firstLine, speedtest_.dlSpeed, speedtest_.ulSpeed, serverLine);
    } else {
        QString res;
        auto content = QCoreApplication::translate(
            "DataViewHtmlGenerator",
...
QString DataViewHtmlGenerator::latencyTestSectionHtml() {
    ...
    auto content = latencyTest_.kind == LatencyTestPanelState::Kind::Url
        ? QCoreApplication::translate("DataViewHtmlGenerator", "Running URL test")
        : QCoreApplication::translate("DataViewHtmlGenerator", "Running IP test");
