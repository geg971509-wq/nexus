...
void StartStopButton::applyState(bool animated) {
    // Idle / Connecting / Running are clickable; Connecting acts as Cancel.
    const bool interactive = (m_state == State::Idle || m_state == State::Connecting || m_state == State::Running);
    ...
    switch (m_state) {
        ...
        case State::Connecting: setToolTip(tr("Cancel start")); break;
...
void StartStopButton::ensureChromeCache() {
    ...
    if (m_state == State::Disconnecting) {
        // Keep the frame looking active while stop is in flight (not clickable).
