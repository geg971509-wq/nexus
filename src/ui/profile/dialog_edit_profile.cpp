...
void DialogEditProfile::typeSelected(const QString &newType) {
    ...
    if (!validType) {
        MessageBoxWarning(newType, tr("Wrong type"));
        return;
    }
    ...
    if (ent->outbound->IsXray()) {
        ...
        ui->xray_downloadsettings_edit->setText(xrayStream->xhttp->downloadSettings.isEmpty() ? tr("Not set") : tr("Already set"));
...
void DialogEditProfile::accept() {
    ...
    if (newEnt) {
        ...
        if (!ok) {
            MessageBoxWarning(tr("Error"), tr("ID already exists"));
            return;
        }
    } else {
...
void DialogEditProfile::editor_cache_updated_impl() {
    ...
    if (ent->outbound->IsXray()) {
        ui->xray_downloadsettings_edit->setText(CACHE.XrayDownloadSettings.isEmpty() ? tr("Not set") : tr("Already set"));
    }
    // CACHE macro
