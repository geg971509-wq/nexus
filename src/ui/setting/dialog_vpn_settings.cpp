...
DialogVPNSettings::DialogVPNSettings(QWidget *parent) : QDialog(parent), ui(new Ui::DialogVPNSettings) {
    ...
    ui->disable_priv_range->setChecked(Configs::dataManager->settingsRepo->private_range_bypass);
    ui->auto_redirect->setChecked(Configs::dataManager->settingsRepo->vpn_auto_redirect);
#ifndef Q_OS_LINUX
    ui->auto_redirect->hide();
#endif
    connect(ui->tun_ipv4_cidr, &QLineEdit::textChanged, this, &DialogVPNSettings::updateTunCidrHint);
    updateTunCidrHint();
    ADJUST_SIZE
}
...
void DialogVPNSettings::updateTunCidrHint() {
    Configs::TunPrivateBypass::V4Cidr cidr{};
    if (!Configs::TunPrivateBypass::parseV4Cidr(ui->tun_ipv4_cidr->text().trimmed(), cidr)) {
        ui->tun_cidr_hint->setText({});
        return;
    }
    if (Configs::TunPrivateBypass::isPrivateSiteIPv4(cidr)) {
        ui->tun_cidr_hint->setText(tr("This Tun CIDR overlaps a private site range. Throne carves it out of LAN bypass automatically so system DNS (Tun+1) stays on Tun."));
    } else {
        ui->tun_cidr_hint->setText({});
    }
}
...
void DialogVPNSettings::accept() {
    ...
    Configs::dataManager->settingsRepo->private_range_bypass = ui->disable_priv_range->isChecked();
