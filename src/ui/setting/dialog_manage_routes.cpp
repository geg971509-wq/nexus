...
void DialogManageRoutes::reloadProfileItems() {
    ...
    // Clear cached sizeHint so long names added after first show aren't elided
    ui->route_prof->setSizeAdjustPolicy(QComboBox::AdjustToContents);
}
...
DialogManageRoutes::DialogManageRoutes(QWidget *parent)
    : QDialog(parent), ui(new Ui::DialogManageRoutes), routeUpdateCancel(std::make_shared<std::atomic_bool>(false)) {
    ui->setupUi(this);
...
#ifdef Q_OS_MACOS
    // Empty underlying DNS breaks Tun on Darwin; soft-fill matches generate auto-default.
    if (ui->local_override->text().trimmed().isEmpty()) {
        ui->local_override->setText(QStringLiteral("1.1.1.1"));
    }
#endif
    ...
    //
    connect(ui->use_dns_object, &QCheckBox::checkStateChanged, this, [this](Qt::CheckState state) {
        const bool useDNSObject = state == Qt::Checked;
        ui->simple_dns_box->setDisabled(useDNSObject);
        ui->dns_object->setDisabled(!useDNSObject);
    });
    emit ui->use_dns_object->checkStateChanged(Qt::Unchecked); // uncheck to uncheck
    connect(ui->dns_document, &QPushButton::clicked, this, [=,this] {
        MessageBoxInfo(tr("DNS"), dnsHelpDocumentUrl);
    });
    connect(ui->format_dns_object, &QPushButton::clicked, this, [=,this] {
        auto obj = QString2QJsonObject(ui->dns_object->toPlainText());
        if (obj.isEmpty()) {
            MessageBoxInfo(tr("DNS"), tr("Invalid JSON"));
        } else {
    ...
    connect(ui->remote_dns, &QComboBox::currentTextChanged, this, [this](const QString&) { updateRemoteDnsHint(); });
    updateRemoteDnsHint();
    ...
    connect(ui->dnshijack_what, &QPushButton::clicked, this, [=,this] {
        MessageBoxInfo(
            tr("What is this?"),
            tr("Listens on the given addr:port (on Windows, port is always 53) and redirects the requests to the DNS module. Domains that match the rules will have their requests hijacked and the A and AAAA queries will be responded with the Inet4 response and Inet6 response respectively.\n"
    ...
    connect(ui->dnshijack_enable, &QCheckBox::checkStateChanged, this, [this](Qt::CheckState state) {
        set_dns_hijack_enability(state == Qt::Checked);
    });
    connect(ui->redirect_enable, &QCheckBox::checkStateChanged, this, [this](Qt::CheckState state) {
        const bool on = state == Qt::Checked;
        ui->redirect_listenaddr->setEnabled(on);
        ui->redirect_listenport->setEnabled(on);
    });
...
void DialogManageRoutes::updateRemoteDnsHint() {
    // Mirrors generate.cpp: bare host / udp / quic remote DNS is upgraded to DoH on proxy path.
    const auto text = ui->remote_dns->currentText().trimmed();
    if (text.isEmpty()) {
        ui->remote_dns_hint->setText({});
        return;
    }
    const bool secureOrAlt =
        text.startsWith("https://") || text.startsWith("h3://") ||
        text.startsWith("tls://") || text.startsWith("tcp://") ||
        text.startsWith("dhcp://") || text.startsWith("local");
    if (secureOrAlt) {
        ui->remote_dns_hint->setText({});
        return;
    }
    ui->remote_dns_hint->setText(
        tr("Runtime upgrades plain UDP/QUIC remote DNS to DoH when detoured via proxy."));
}

DialogManageRoutes::~DialogManageRoutes() {
    routeUpdateCancel->store(true, std::memory_order_release);
    delete ui;
}
...
void DialogManageRoutes::on_export_route_clicked()
{
    ...
    QTimer::singleShot(1500, [self, r] {
        if (!self || self->tooltipID != r) return;
        QToolTip::hideText();
    });
}
...
void DialogManageRoutes::updateRemoteProfiles(const QList<std::shared_ptr<Configs::RouteProfile>>& profiles) {
    ...
    routeUpdateCancel->store(false, std::memory_order_release);
    ...
    QPointer<DialogManageRoutes> self(this);
    const auto cancel = routeUpdateCancel;
    runOnNewThread([profiles, total, progressText, cancel, self] {
        QStringList failures;
        int ok = 0;
        for (int i = 0; i < profiles.size(); ++i) {
            if (cancel->load(std::memory_order_acquire)) break;
            const int current = i + 1;
            runOnUiThread([self, progressText, current, cancel] {
                if (self && self->routeUpdateRunning && !cancel->load(std::memory_order_acquire))
                    self->ui->update_route->setText(progressText(current));
            });
            QString warnings;
        ...
        const bool cancelled = cancel->load(std::memory_order_acquire);
        runOnUiThread([self, failures, ok, total, cancelled] {
            if (!self) return;
            self->routeUpdateRunning = false;
            self->ui->update_route->setText(tr("Update"));
            self->reloadProfileItems();
            if (cancelled) {
