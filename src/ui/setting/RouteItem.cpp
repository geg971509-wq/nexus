...
void adjustComboBoxWidth(QComboBox *comboBox) {
    ...
    const int chrome = comboBox->style()->pixelMetric(QStyle::PM_ScrollBarExtent) + 12;
    comboBox->view()->setMinimumWidth(maxWidth + chrome);
    comboBox->setMinimumWidth(maxWidth + chrome);
}
...
RouteItem::RouteItem(QWidget *parent, const std::shared_ptr<Configs::RouteProfile>& routeChain)
    ...
    bool hasLegacyWarp = chain->defaultOutboundID == Configs::warpBypassID
                      || !chain->GetSimpleRules(Configs::warpBypass).isEmpty();
    for (const auto& rule : chain->Rules) {
        if (rule->outboundID == Configs::warpBypassID) {
            hasLegacyWarp = true;
            break;
        }
    }
    ...
    outboundMap[1] = -2;
    if (hasLegacyWarp) {
        outbounds << "warp-bypass";
        outboundMap[2] = Configs::warpBypassID;
    } else {
        ui->simple_warpbypass_box->hide();
        if (auto index = ui->def_out->findText("warp-bypass"); index >= 0) {
            ui->def_out->removeItem(index);
        }
    }
    ...
    connect(ui->howtouse_button, &QPushButton::clicked, this, [=]() {
        runOnUiThread([=] {
            MessageBoxInfo(
                tr("Simple rule manual"),
                tr("You can add rules with the following format:\n"
                   "domain:<your-domain>\n"
...
void RouteItem::fetchRemote(bool applyToChain) {
    ...
    QPointer<RouteItem> self(this);
    const auto sourceChain = chain;
    runOnNewThread([applyToChain, sourceChain, url, currentName, origFetch, origPreview, self] {
        // For a preview we work on a throwaway copy so the editor's rules are untouched.
        auto target = applyToChain ? sourceChain : std::make_shared<Configs::RouteProfile>(*sourceChain);
        ...
        runOnUiThread([self, applyToChain, target, warnings, err, origFetch, origPreview] {
            if (!self) return;
            self->ui->remotePreviewBtn->setEnabled(true);
            self->ui->remoteFetchBtn->setEnabled(true);
            self->ui->remoteFetchBtn->setText(origFetch);
            self->ui->remotePreviewBtn->setText(origPreview);
            if (!err.isEmpty()) {
                MessageBoxWarning(tr("Could not fetch routing profile"), err);
                return;
            }
            if (applyToChain) {
                self->reloadRuleViewsFromChain();
