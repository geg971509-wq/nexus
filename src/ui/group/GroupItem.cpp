...
void GroupItem::on_remove_clicked() {
    ...
    if (QMessageBox::question(this, tr("Confirmation"), tr("Remove %1?").arg(ent->name)) ==
        ...
        if (!GetMainWindow()->profile_stop(false, true, false)) return;
        if (!Configs::dataManager->groupsRepo->DeleteGroup(ent->id)) {
            MessageBoxWarning(tr("Remove group"), tr("Failed to remove %1.").arg(ent->name));
            return;
        }
        MW_dialog_message(MwMessage::GroupsChanged, {});
        delete item;
