...
private:
    ...
    // True while an update batch is in flight (UI-thread only). routeUpdateCancel is shared with
    // the worker so the dialog may be destroyed before the worker exits.
    bool routeUpdateRunning = false;
    std::shared_ptr<std::atomic_bool> routeUpdateCancel;
