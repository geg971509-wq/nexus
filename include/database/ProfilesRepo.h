...
namespace Configs {
    ...
    public:
        ...
        // When stopRunningProfile is false, the currently started profile is
        // excluded from the deletion set. When true, it is deleted as well and
        // *outDeletedRunningProfile (if given) is set so the UI caller can stop
        // the running profile — this repo layer does not call into the UI.
        bool BatchDeleteProfiles(QList<int>& ids, bool stopRunningProfile = false, bool* outDeletedRunningProfile = nullptr);

        // Atomic sub_clear replace: insert new profiles, rewrite group profile list,
        // delete old profiles in one SQLite transaction. Rejects empty newProfiles.
        // On failure: DB rolled back; memory/group lists restored; returns false.
        // keep holds (position, id) pairs of existing profiles the subscription
        // does not own (auto selectors); they survive the replace and are put back
        // at their positions in the rewritten group list.
        bool ReplaceGroupProfiles(int gid, QList<std::shared_ptr<Profile>>& newProfiles,
