...
namespace Configs {
    ...
    public:
        ...
        // Inserts the group and its tab-order row in one transaction. On failure
        // nothing is written, group->id stays -1, and false is returned.
        [[nodiscard]] bool AddGroup(std::shared_ptr<Group>& group);
        ...
        // Delete group from database. Both rows go in one transaction; returns
        // false (leaving the cache untouched) when the write fails.
        [[nodiscard]] bool DeleteGroup(int id);
