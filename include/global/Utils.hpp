...
// Wait until every finite task launched by runOnNewThread() has finished.
// The UI event loop is pumped while waiting so tasks using runOnUiThread(..., true)
// can complete without deadlocking shutdown.
